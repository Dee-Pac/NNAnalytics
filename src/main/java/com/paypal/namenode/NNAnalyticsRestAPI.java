 ...
 *
 * It is best described as: “A modified, isolated, read-only, Standby NameNode, with no RPC Server,
 * and with a Web Server and custom query engine embedded inside it”.
 *
 * NNA is a utility to provide real-time queries against an active HDFS instance without having to
 * touch the active cluster itself. It is meant to replace the old practice of parsing an FSImage
 * offline using a legacy FSImage and OfflineImageViewer (OIV) tool.
 *
 * The goal for NNA is to be able aid cluster administrators in visualizing how data is being used
 * throughout their cluster via a simple query API.
 *
 * NNA is bootstrap'd via a one-time cost of fetching the latest FSImage from the active cluster and
 * then utilizing that cluster's JournalNodes in order to stay up-to-date. Queries can be ran
...
public class NNAnalyticsRestAPI {
  ...
  public void initAuth(Boolean overrideAuthentication,
    ...
    if (ldapEnabled) {
      ...
      // jwt:
      SignatureConfiguration sigConf = new SecretSignatureConfiguration(
          secConf.getJwtSignatureSecret());
      EncryptionConfiguration encConf = new SecretEncryptionConfiguration(
          secConf.getJwtEncryptionSecret(),
   ...
   *
   * It is made public for those wishing to have control of the Rest Server start-up of NNA
   * instances in tests.
   */
  @VisibleForTesting
  public void initRestServer() {
    ...
    get("/system", (req, res) -> {
      ...
      sb.append("Non-Heap Committed Memory (KB): ").append((memNonHeap.getCommitted() / 1024))
          .append("\n");
      ...
      sb.append("Heap Committed Memory (KB): ").append((memHeap.getCommitted() / 1024))
          .append("\n");
    ...
    get("/info", (req, res) -> {
      ...
      if (isInit) {
        ...
        sb.append("INodeFile set percentage: ").append(((fileSetSize * 100.0f) / allSetSize))
            .append("\n\n");
        sb.append("INodeDir set size: ").append(dirSetSize).append("\n");
        sb.append("INodeDir set percentage: ").append(((dirSetSize * 100.0f) / allSetSize))
            .append("\n\n");
      }
    ...
    get("/threads", (req, res) -> {
      ...
      return "New Threads: " + threadsNew + "\n" +
          "Runnable Threads: " + threadsRunnable + "\n" +
          "Blocked Threads: " + threadsBlocked + "\n" +
          "Waiting Threads: " + threadsWaiting + "\n" +
          "Timed-Waiting Threads: " + threadsTimedWaiting + "\n" +
          "Terminated Threads: " + threadsTerminated + "\n";
    });
    ...
    get("/divide", (req, res) -> {
      ...
      try {
        ...
        Collection<INode> inodes1 = NNAHelper.performFilters(nnLoader, set1, filters1, filterOps1);
        Collection<INode> inodes2 = NNAHelper.performFilters(nnLoader, set2, filters2, filterOps2);
    ...
    get("/filter", (req, res) -> {
      ...
      try {
        ...
        Collection<INode> inodes = NNAHelper
            .performFilters(nnLoader, set, filters, filterOps, find);
        ...
        if (sums.length == 1 && sumStr != null) {
          ...
          if (emailsTo != null && emailsTo.length != 0 && emailHost != null && emailFrom != null) {
            String subject = nnLoader.getAuthority() + " | " + sum + " | " + set + " | Filters: "
                + fullFilterStr;
            try {
              if (emailConditionsStr != null) {
    ...
    get("/histogram", (req, res) -> {
      ...
      try {
        ...
        Map<String, Function<INode, Long>> transformMap =
            Transforms.getAttributeTransforms(transformConditionsStr, transformFieldsStr,
                transformOutputsStr, nnLoader);
        ...
        try {
          switch (htEnum) {
            ...
            case dirQuota:
              ...
              break;
            default:
              throw new IllegalArgumentException("Could not determine histogram type: " + histType +
        ...
        if (emailsTo != null && emailsTo.length != 0 && emailHost != null && emailFrom != null) {
          String subject =
              nnLoader.getAuthority() + " | X: " + histType + " | Y: " + sum + " | " + set
                  + " | Filters: " + fullFilterStr;
          try {
        ...
        switch (output) {
          case chart:
            res.header("Content-Type", "application/json");
            return Histograms
                .toChartJsJson(histogram, NNAHelper.toTitle(histType, sum), NNAHelper.toYAxis(sum),
                    xAxis);
          case json:
            res.header("Content-Type", "application/json");
          ...
          default:
            throw new IllegalArgumentException("Could not determine output type: " + histType +
    ...
    get("/histogram2", (req, res) -> {
      ...
      try {
            ...
            switch (htEnum) {
              ...
              default:
                throw new IllegalArgumentException(
                    "Could not determine histogram type: " + histType +
                        ".\nPlease check /histograms for available histograms.");
            }
          } finally {
            nnLoader.namesystemWriteUnlock(useLock);
        ...
        Map<String, List<Long>> mergedHistogram = histograms.parallelStream()
      ...
      try {
        ...
        switch (operationSplits[0]) {
          ...
          case "setReplication":
            ...
            operationObj = new SetReplication(inodes, req.queryString(), secContext.getUserName(),
                fs,
                newReplFactor);
            break;
          case "setStoragePolicy":
            operationObj = new SetStoragePolicy(inodes, req.queryString(), secContext.getUserName(),
                fs,
                operationSplits[1]);
            break;
          default:
            throw new IllegalArgumentException("Unknown operation:" + operationSplits[0] +
                ". Please check /operations.");
        }
    ...
    get("/listOperations", (req, res) -> {
      ...
      try {
        ...
        if (identity == null || identity.length() == 0) {
          ...
          String sb = "Identity: " + operation.identity() + '\n' +
              "Owner: " + operation.owner() + '\n' +
              "Query: " + operation.query() + '\n' +
              "Total to perform: " + totalToPerform + '\n' +
              "Total performed: " + numPerformed + '\n' +
              "Total left to perform: " + (totalToPerform - numPerformed) + '\n' +
              "Percentage done: " + percentageDone + '\n' +
              "Up next: " + operation.upNext() + '\n' +
              "Last (" + limit + ") performed: " + lastDeleted.toString() + "\n\n";
          res.body(sb);
        }
        return res;
      } finally {
    ...
    get("/abortOperation", (req, res) -> {
      ...
      try {
        ...
        String sb = "Identity: " + operation.identity() + '\n' +
            ...
            "Percentage done: " + percentageDone + '\n' +
            "Up next: " + operation.upNext() + '\n' +
            "Last (" + limit + ") performed: " + lastPerformed.toString() + "\n\n";
        res.body(sb);
        return res;
      } finally {
        lock.writeLock().unlock();
    ...
    get("/token", (req, res) -> {
      ...
      return nnLoader.getSuggestionsEngine().getTokens();
        ...
        /* SAVENAMESPACE endpoint is an admin-level endpoint meant to dump the in-memory INode set
           to a fresh FSImage. */
    get("/saveNamespace", (req, res) -> {
      res.header("Access-Control-Allow-Origin", "*");
      res.header("Content-Type", "text/plain");
      String dirStr = req.queryMap("dir").value();
      String dir = (dirStr != null) ? dirStr :
          "/usr/local/nn-analytics/dfs/name/legacy/";
